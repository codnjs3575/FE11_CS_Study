...
이러한 디자인 패턴의 특성으로 인해, 디자인 패턴을 활용하면 개발하면서 마주하게 되는 새로운 문제들에 비교적 쉽고 빠르게 대응이 가능해지는 것이다.
...
button.textContent = '클릭해보기';
document.body.appendChild(button);

// Observer
function handleClickA() {
  console.log('옵저버 A: 버튼이 클릭되었다!');
}

function handleClickB() {
  console.log('옵저버 B: 버튼 클릭 감지했다!');
}
